...
Public Class SeatManager
    ...
    Public Function GetNumVacant() As Integer
        Dim amount As Integer = 0
        For i As Integer = 0 To mTotalSeats - 1 Step 1
            If IsVacantAt(i) Then 'If the IsVacantAt-function returns true, the counter goes up!
                amount += 1
            End If
        Next
        Return amount
    ...
    End Function
    ''' <summary>
    ''' Used to check if a seat is reserved at the specified index
    ''' </summary>
    ''' <param name="index">
    ''' The specified index in the mNameList to be checked
    ''' </param>
    ''' <returns>
    ''' True if the string at the index isn't empty
    ''' </returns>
    Public Function IsReservedAt(ByVal index As Integer) As Boolean
        Return Not String.IsNullOrEmpty(mNameList(index))
    End Function
    ''' <summary>
    ''' Used to check if a seat is vacant at the specified index
    ''' </summary>
    ''' <param name="index">
    ''' The specified index in the mNameList to be checked
    ''' </param>
    ''' <returns>
    ''' True if the string at the index IS empty
    ''' </returns>
    Private Function IsVacantAt(ByVal index As Integer) As Boolean
        Return String.IsNullOrEmpty(mNameList(index))
    End Function
